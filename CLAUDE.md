...
core/
  ...
  core-ui/              → Material 3 theme, reusable Compose components
feature/
  feature-auth/         → Login/register
  feature-dashboard/    → Camera grid view
  feature-player/       → RTSP video player + PTZ control
  feature-devices/      → Device management, ONVIF discovery, QR scanning
  feature-access-control/ → Access control + QR code generation
...
- **Navigation:** Jetpack Navigation Compose with string routes (`"login"`, `"dashboard"`, `"player/$cameraId"`, `"settings"`). Start destination: `"login"`.
- **Networking:** Supabase SDK over Ktor. `SUPABASE_URL` and `SUPABASE_KEY` injected via BuildConfig.
- **Persistence:** Room (schema export to `core/core-data/schemas/`) + DataStore Preferences.
